...
# 安装项目依赖
npm install
# 首次启动时配置环境变量；如果 .env 已存在，不要重复覆盖
test -f .env || cp .env.example .env
# 编辑 .env，填入 API Key 和可用的模型/推理接入点
# 同步数据库结构
npx prisma db push
...
npm run start:dev
```
*注意：默认使用火山引擎方舟。请在 .env 文件中填入有效的 ARK_API_KEY 或 LLM_API_KEY，并确认 LLM_MODEL/ARK_MODEL 是方舟控制台里已开通的模型，或 ep- 开头的推理接入点 ID。RAG 向量化默认使用 Ark/豆包 embedding，可通过 EMBEDDING_MODEL 配置模型或推理接入点。*
