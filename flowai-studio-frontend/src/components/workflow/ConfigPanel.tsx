...
import { AutoComplete, Form, Input, Select, Slider, Typography, Empty } from 'antd'
...
const ConfigPanel: React.FC = () => {
  ...
  const renderConfigForm = () => {
    ...
    const commonFields = (
      <>
        <div className="config-node-id" title={selectedNode.id}>
          节点 ID：<span>{selectedNode.id}</span>
        </div>
    ...
    switch (selectedNode.type) {
      ...
      case 'llm':
        return (
          <>
            {commonFields}
            <Form.Item name="model" label="模型" initialValue="doubao-seed-1-6-250615">
              <AutoComplete
                options={[
