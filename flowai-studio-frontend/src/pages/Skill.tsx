...
const Skill: React.FC = () => {
  ...
  const [codeToolResult, setCodeToolResult] = useState<any>(null)
  ...
  const runCodeTool = async (endpoint: 'list' | 'read' | 'search', payload: Record<string, any>) => {
    setCodeToolLoading(true)
    try {
      const response = await request.post(`/skill/code/${endpoint}`, payload) as any
      setCodeToolResult(response.data)
    } catch (error: any) {
      message.error(error?.response?.data?.message || '代码工具执行失败')
    } finally {
  ...
  return (
    <div className="skill-page">
      ...
      <section className="code-tool-panel">
        <div className="code-tool-header">
          <div>
            <h3 className="code-tool-title"><ToolOutlined /> 本地代码读取</h3>
            <p className="code-tool-subtitle">仅允许访问当前工作区，自动跳过 node_modules、.git、dist 等目录。</p>
          </div>
        </div>
        <div className="code-tool-grid">
          <div className="code-tool-action">
            <label>列目录</label>
            <Input
              value={codePath}
              onChange={(e) => setCodePath(e.target.value)}
              placeholder="例如：flowai-studio-backend/src"
              prefix={<FolderOpenOutlined />}
            />
            <Button
              icon={<FolderOpenOutlined />}
              loading={codeToolLoading}
              onClick={() => runCodeTool('list', { path: codePath })}
            >
              列出
            </Button>
          </div>
          <div className="code-tool-action">
            ...
            <Button
              ...
              loading={codeToolLoading}
              onClick={() => runCodeTool('read', { path: readPath })}
            >
              读取
            </Button>
          </div>
          <div className="code-tool-action">
            ...
            <Input
              ...
              placeholder="例如：chatWithLLM"
              prefix={<SearchOutlined />}
              onPressEnter={() => searchQuery.trim() && runCodeTool('search', { query: searchQuery, path: codePath })}
            />
            <Button
              icon={<FileSearchOutlined />}
              loading={codeToolLoading}
              onClick={() => runCodeTool('search', { query: searchQuery, path: codePath })}
            >
              搜索
            </Button>
          </div>
        ...
        {codeToolResult && (
          <pre className="code-tool-result">
            {typeof codeToolResult.content === 'string'
              ? codeToolResult.content
              : JSON.stringify(codeToolResult, null, 2)}
          </pre>
        )}
      </section>
