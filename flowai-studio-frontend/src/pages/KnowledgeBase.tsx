...
const KnowledgeBase: React.FC = () => {
  ...
  const [repoImport, setRepoImport] = useState({
    path: '.',
    extensions: '.ts,.tsx,.js,.jsx,.css,.md,.json,.prisma',
    maxFiles: 30,
  })
  ...
  const handleImportCodeRepository = async () => {
    if (!selectedKb?.id) return
    if (!repoImport.path.trim()) {
      message.error('请输入仓库路径')
      return
    }
    if (repoImport.path.includes('ENOENT') || repoImport.path.includes('no such file or directory')) {
      message.error('仓库路径里包含错误信息，请只填写目录路径，例如 flowai-studio-backend/src')
      return
    }

    setRepoImporting(true)
    try {
      const result = await importCodeRepository({
        knowledgeBaseId: selectedKb.id,
        path: repoImport.path.trim(),
        extensions: repoImport.extensions
          .split(',')
          .map((ext) => ext.trim())
          .filter(Boolean),
        maxFiles: repoImport.maxFiles,
      })
      message.success(`已导入 ${result.importedFiles} 个文件，生成 ${result.chunks} 个代码分块`)
      const updatedKb = await fetchKnowledgeBaseById(selectedKb.id)
      setDocuments(updatedKb.documents || [])
    } catch (error) {
      message.error((error as any)?.response?.data?.message || '代码仓库导入失败')
    } finally {
      ...
      >
        <div className="doc-modal-body">
          ...
          <Dragger
            name="file"
            multiple={false}
            accept=".txt,.md,.markdown,.json,.csv,.log,.yaml,.yml,.pdf,application/pdf,text/plain,text/markdown,application/json"
          ...
          <div className="code-import-panel">
            <div className="code-import-header">
              <div>
                <Text strong><CodeOutlined /> 导入代码仓库</Text>
                <div className="code-import-hint">扫描当前工作区内的源码文件，生成向量分块后写入当前知识库。</div>
              </div>
              <Button
                icon={<CodeOutlined />}
                loading={repoImporting}
                onClick={handleImportCodeRepository}
              >
                导入
              </Button>
            </div>
            <div className="code-import-grid">
              <div className="kb-field">
                <label className="kb-field-label">仓库路径</label>
                <Input
                  value={repoImport.path}
                  onChange={(e) => setRepoImport({ ...repoImport, path: e.target.value })}
                  placeholder="例如：. 或 flowai-studio-backend/src"
                />
              </div>
              <div className="kb-field">
                <label className="kb-field-label">扩展名</label>
                <Input
                  value={repoImport.extensions}
                  onChange={(e) => setRepoImport({ ...repoImport, extensions: e.target.value })}
                  placeholder=".ts,.tsx,.js,.md"
                />
              </div>
              <div className="kb-field">
                <label className="kb-field-label">最多文件数</label>
                <Input
                  type="number"
                  min={1}
                  max={300}
                  value={repoImport.maxFiles}
                  onChange={(e) => setRepoImport({ ...repoImport, maxFiles: Number(e.target.value || 80) })}
                />
              </div>
            </div>
          </div>
