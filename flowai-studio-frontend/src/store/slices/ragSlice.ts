...
export const createRAGSlice: StateCreator<RAGSlice> = (set, get) => ({
  ...
  importCodeRepository: async (data) => {
    set({ isLoading: true, error: null })
    try {
      const response = await request.post('/rag/code-repo/import', data, {
        timeout: 300000,
      }) as any
      await get().fetchKnowledgeBases()
      if (get().currentKnowledgeBase?.id === data.knowledgeBaseId) {
        await get().fetchKnowledgeBaseById(data.knowledgeBaseId)
      }
      set({ isLoading: false })
